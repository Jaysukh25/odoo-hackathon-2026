...
@section('content')
    ...
    <div class="row mb-4">
        <div class="col-xl-3 col-md-6 mb-4">
            <div class="kpi-card">
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        <div class="kpi-number">{{ $activeVehicles }}</div>
        ...
        <div class="col-xl-3 col-md-6 mb-4">
            <div class="kpi-card" style="background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%);">
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        <div class="kpi-number">{{ $tripsToday }}</div>
        ...
        <div class="col-xl-3 col-md-6 mb-4">
            <div class="kpi-card" style="background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%);">
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        <div class="kpi-number">{{ $vehiclesInShop }}</div>
        ...
        <div class="col-xl-3 col-md-6 mb-4">
            <div class="kpi-card" style="background: linear-gradient(135deg, #43e97b 0%, #38f9d7 100%);">
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        <div class="kpi-number">${{ number_format($monthlyOperationalCost, 0) }}</div>
    ...
    @if($maintenanceAlerts->count() > 0)
        <div class="alert alert-warning alert-dismissible fade show" role="alert">
            ...
            <strong>Predictive Maintenance Alert:</strong> {{ $maintenanceAlerts->count() }} vehicle(s) require maintenance
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-hover">
                            ...
                            <tbody>
                                @forelse($liveFleet as $fleet)
                                    <tr>
                                        <td>
                                            <div class="d-flex align-items-center">
                                                <i class="bi bi-truck me-2 text-primary"></i>
                                                <div>
                                                    <div class="fw-semibold">{{ $fleet['model'] }}</div>
                                                    <small class="text-muted">{{ $fleet['license_plate'] }}</small>
                                                </div>
                                            </div>
                                        </td>
                                        <td>{{ $fleet['driver'] }}</td>
                                        <td>
                                            <span class="status-badge status-{{ $fleet['status'] }}">
                                                {{ ucfirst(str_replace('_', ' ', $fleet['status'])) }}
                                            </span>
                                        </td>
                                        <td>{{ number_format($fleet['load'], 2) }} kg</td>
                                        <td>{{ $fleet['destination'] }}</td>
                                        <td>
                                            <button class="btn btn-sm btn-outline-primary fleet-view-btn"
                                                data-id="{{ $fleet['id'] }}" title="View Details">
                                                <i class="bi bi-eye"></i>
                                            </button>
                                        </td>
                                    </tr>
...
@endsection
    ...
    @section('scripts')
        <script>
            // ── Fleet Detail Modal ──────────────────────────────────────────
            const FLEET_ENDPOINT = '{{ route("dashboard.fleet.detail", ":id") }}';
            ...
            const tripActivityChart = new Chart(tripActivityCtx, {
                type: 'bar',
                data: {
                    labels: @json($tripActivity->pluck('date')->map(fn($date) => \Carbon\Carbon::parse($date)->format('M d'))),
                    datasets: [{
                        label: 'Trips',
                        data: @json($tripActivity->pluck('count')),
            ...
            const driverSafetyChart = new Chart(driverSafetyCtx, {
                ...
                data: {
                    ...
                    datasets: [{
                        data: [{{ $driverSafetyData['SAFE'] }}, {{ $driverSafetyData['MODERATE'] }}, {{ $driverSafetyData['RISKY'] }}],
